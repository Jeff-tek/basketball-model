...
def build_tip(
    ...
    Ensemble averages all ML opinions; spread/total from Normal only.
    Safest pick across all 6 markets.  Verdict from probability bands;
    models disagreeing (ML spread > AGREE_GAP) downgrades one band.
    ...
    # Ensemble ML across voting models
    ...
    ens_ml = {k: sum(v[k] for v in votes.values()) / len(votes)
              for k in ("home", "away")}
    ...
    gap = max(
        max(v[k] for v in votes.values()) - min(v[k] for v in votes.values())
        for k in ("home", "away")
    )
    agree = gap <= AGREE_GAP
