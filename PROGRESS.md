...
- VIZ 2026-09-15 basketball spider-chart port (football parity): NEW `frontend/app/components/ModelVisuals.tsx` (OddsRadar 5-axis Home/Away/Cover/Over/Under + CrowdBars H/A model-vs-crowd), NEW `frontend/app/components/TeamDuel.tsx` (Rank/W-L/Win%/Streak/Pace/PF-PA/Form table from teamMeta+pace), TipCard.tsx wires viz-stack (radar + duel-row) matching football layout, globals.css += viz-block/radar/duel/team-table styles in court colors, tips.ts reshaped to {ml,spread,totals}+fair+bookOdds{ml_home,ml_away,spread,spread_price,total,over_price,under_price}+TeamMeta{rank,wins,losses,winPct,streak,pf,pa}, server/main.py += _to_num/_to_int_or_none/_standings_map/_team_meta/_pace_for_game + fair/edge/teamMeta/pace in /tips envelope (never raises). Verified: py_compile OK, edge propagated (TipCard t.edge.value safe), Vercel build is the typecheck per no-local-install loop.
